...
const CartPage = () => {
    ...
    const handlePayment=async()=>{
        setLoading(true)
            ...
            await axios({
                method:'POST',
                url:'http://localhost:8080/api/v1/product/braintree/payment',
                data:{nonce,cart}
            }).then((res)=>{
                setLoading(false)
                ...
                <div className='row'>
                    <div className='col-md-7'>
                        ...
                        <div className='row mb-2 p-3 card flex-row'>
                            <div className='col-md-4'><img src={`http://localhost:8080/api/v1/product/product-photo/${p._id}`} className="card-img-top" alt={p.name} width={"10px"} height="150px" /></div>
                            <div className='col-md-8 mt-3'>
                                <h4>{p.name}</h4>
                                <p>{p.description.substring(0,30)}</p>
                                <p>Price: ${p.price}</p>
                                <RiDeleteBinFill type='submit' size={20} onClick={()=>removeCartItem(p._id)} />
